...
This example shows you that symbolic execution is not much different from ordinary \(concrete\) execution. The main difference is that at some point of the execution, because of the presence of symbolic values, it might become unclear what is the next thing to do. In this case it is necessary to introduce an assumption on the possible values of the symbolic inputs so the next action is unambiguously identified. Such an assumption is called the _path condition_, because it is progressively built as symbolic execution explores a path in the program. All the input values that satisfy \(i.e., solve\) a path condition drive the execution of the program through the same path: For instance, all the input values $$x_0$$ for the "double-if" program that satisfy the condition $$x_0 \leq 0$$ drive the program execution through the `else` branch of the two `if` statements. Conversely, if a path condition has no solution, then no program inputs drive the program through the corresponding path: The path is said to be unfeasible. For instance, the path in the "double-if" program that goes through the then branch of the first if statement and the else branch of the second if statement is $$x_0 > 0 \land x_0 \leq 0$$, that is clearly unsatisfiable. Correspondingly, no input exist that drive the program through this path.
