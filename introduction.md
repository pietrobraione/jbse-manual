...
* Once finished the analysis of the case $$x_0 > 0$$ JBSE _backtracks_, i.e., restores the state of the execution where the next statement to be executed is the first if statement, and considers the opposite case, i.e., the case where branch condition `x > 0` evaluates to `false`. Since in the backtracked state it is again `x == `$$x_0$$, this happens when $$x_0 \leq 0$$.
