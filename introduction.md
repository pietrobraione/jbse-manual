...
JBSE performs a kind of dynamic analysis that is called _symbolic execution_, that is amenable both to verify the correctness of a program with respect to some desired properties expressed as assertions, and to generate test vectors for the program. When used for verification JBSE expects that you specify the verification properties of interest for your project as a set of _assumptions _and _assertions_. Assumptions specify the conditions that must be satisfied for an execution to be relevant. Preconditions are a typical form of assumptions, specifying the range of values for symbolic inputs. Assertions specify the conditions that must be satisfied for an execution to be correct. JBSE attempts to determine whether some inputs exist that satisfy all the assumptions and falsify at least one assertion. In this regard JBSE is more similar in spirit, implementation and mode of use to tools like [Symbolic PathFinder](http://babelfish.arc.nasa.gov/trac/jpf/wiki/projects/jpf-symbc), [Sireum/Kiasan](http://www.sireum.org/) and [JNuke](http://fmv.jku.at/jnuke/).
...
This example shows you that symbolic execution is not much different from ordinary \(concrete\) execution of programs. The main difference is that at some point of a symbolic execution, because of the presence of symbolic values, it might become unclear what is the next thing to do. In this case it is necessary to introduce an assumption on the possible values of the symbolic inputs so the next action is unambiguously identified. Such an assumption is called the _path condition_, because it is progressively built as symbolic execution explores a path in the control flow graph of the program. All the input values that satisfy \(i.e., solve\) a path condition drive the execution of the program through the same path: For instance, all the input values $$x_0$$ for the "double-if" program that satisfy the condition $$x_0 \leq 0$$ drive the program execution through the `else` branches of the two `if` statements. Conversely, if a path condition has no solution, then no program inputs drive the program through the corresponding path: The path is said to be _unfeasible_. For instance, the path in the "double-if" program that goes through the then branch of the first if statement and the else branch of the second if statement is $$x_0 > 0 \land x_0 \leq 0$$, that is clearly unsatisfiable. Correspondingly, no input exists that drives the program through this path.
