...
![Symbolic execution tree for the "double-if" program (only feasible paths)](/assets/doubleif_symbolic_tree.svg)

The previous figure reports the symbolic execution tree for the "double-if" program. Circles are program states, indicating the values stored for all the variables in the program. Arrows join a state with its possible successors, and are labeled according to the next statement to be executed: If this is an assignment, the label reports the assignment, if it is a conditional, the label reports the _evaluation_ of the conditional in the pre-state. The final states that pass the assertion are represented by a green tick. The figure does not show the infeasible paths, but we will often consider the case of symbolic execution trees where also infeasible paths are reported. The next figure, for instance, is obtained by the previous by adding the two omitted infeasible paths:

![Symbolic execution tree for the "double-if" program (feasible and unfeasible paths)](/assets/doubleif_symbolic_tree_with_unfeasible.svg)

The infeasible paths end in a red cross, signifying a final state that does not pass the assertion. The path condition for a certain path is obtained by visiting the symbolic execution tree from the root and conjoining all the edge labels for conditional expressions evaluations.

![A path in the "double-if" program and its path condition](/assets/doubleif_symbolic_tree_path.svg)

In the previous figure the path condition for the path marked with the red dashed arrow is given by conjoining the two expressions circled in red, yielding $$x_0 > 0 \land x_0 \leq 0$$. Being the path condition unsatisfiable, the path is unfeasible.
