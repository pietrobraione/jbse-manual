...
.. figure:: ../img/doubleif_symbolic_tree.svg

   Symbolic execution tree for the "double-if" program (only feasible paths).
...
.. figure:: ../img/doubleif_symbolic_tree_with_unfeasible.svg

   Symbolic execution tree for the "double-if" program (feasible and unfeasible paths.
...
.. figure:: ../img/doubleif_symbolic_tree_path.svg

   A path in the "double-if" program and its path condition.

In the previous figure the path condition for the path marked with the red dashed arrow is given by conjoining the two expressions circled in red, yielding :math:`x_0 > 0 \land x_0 \leq 0`. Being the path condition unsatisfiable, the path is unfeasible.
